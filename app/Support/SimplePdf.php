...
class SimplePdf
{
    ...
    /**
     * Lebar glyph Helvetica per 1000 unit em (dari metrik AFM standar).
     *
     * Dipakai agar penempatan teks berdampingan — misalnya daftar pilihan
     * "Cukup / Belum Cukup" beserta coretannya — tidak saling menempel atau
     * melewati batas seperti bila lebarnya hanya ditaksir per karakter.
     */
    ...
    /**
     * Lebar teks sebenarnya pada font Helvetica, dalam satuan poin.
     */
    public function textWidth(string $text, float $size = 10): float
    {
        $normalized = $this->normalize($text);
        $total = 0;

        for ($i = 0, $length = strlen($normalized); $i < $length; $i++) {
            // Glyph di luar tabel (mis. karakter Windows-1252) memakai lebar rata-rata.
            $total += self::HELVETICA_WIDTHS[ord($normalized[$i])] ?? 556;
        }
